...
const { PageSection, PageSectionInner, InnerWrapper, FormWrapper } =
  HomePageStyles;

const { Text48Orbitron700, Text24Zekton400 } = TypographyComponents;

const { Text16Zekton400Black } = OverrideTypographyComponents;

const { AlertImage, CheckImage } = ImageViewComponents;
...
export const UserForm = forwardRef<null, userFormProps>(
  ({ subscribeRef }, ref) => {
    ...
    const handleSendEmail = useCallback(async () => {
      try {
        ...
        let sendpulseAddUser =
          "https://red-morning-c59a.attackcyberpunk.workers.dev/?listId=2045";
            ...
            <>
              ...
              <InnerWrapper>
                <Text24Zekton400 color="white">
                  {t("section1.thanksText")}
                </Text24Zekton400>
              </InnerWrapper>
              <FormWrapper $isStretch>
                <SubmitFormButton onPress={handleReload}>
                  <Text16Zekton400Black>{t("reload")}</Text16Zekton400Black>
                </SubmitFormButton>
              </FormWrapper>
            </>
          ) : (
